...
function WordCloudTimeline() {
  ...
  // Load data (API or mock) when component mounts or team/mode changes
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    setCurrentDayIndex(0); // Reset to first day when changing data source

    if (useApi) {
      // Fetch from API - get all available dates for the team
      const today = new Date();
      const dates = [];

      // Generate last 7 days of dates to fetch
      for (let i = 6; i >= 0; i--) {
      ...
      Promise.all(
        dates.map(dateStr =>
          fetch(`http://localhost:8000/api/keywords/${selectedTeam}/${dateStr}`)
            .then(response => {
              if (!response.ok) {
                // If date doesn't have data, return null
        ...
        .then(results => {
          // Combine all keywords from all successful fetches
          const allKeywords = results
            .filter(result => result && result.keywords)
          ...
          setData({ keywords: allKeywords });
          setLoading(false);
        })
        .catch(err => {
          console.error('WordCloudTimeline: Error loading API data:', err);
      ...
      fetch(`/${teamConfig.file}`)
        ...
        .catch(err => {
          console.error('WordCloudTimeline: Error loading mock data:', err);
          setError(err.message);
          setLoading(false);
        });
    }
  }, [selectedTeam, useApi, teams]);

  // Auto-play functionality
  useEffect(() => {
    if (isPlaying && data) {
      const uniqueDatesCount = [...new Set(data.keywords.map(kw => kw.date))]
        .length;
  ...
  useEffect(() => {
    ...
    const uniqueDates = [...new Set(keywords.map(kw => kw.date))].sort();
    if (currentDayIndex >= uniqueDates.length) return;
    const targetDate = uniqueDates[currentDayIndex];

    // Get keywords for the current date
    const currentDateKeywords = keywords.filter(kw => kw.date === targetDate);
    ...
    const renderWordCloud = words => {
      ...
      function draw(cloudWords) {
        ...
        // ENTER: new words with event handlers attached immediately
        ...
        // UPDATE: transition existing words to new position/size/color
        textElements
          .transition()
          .duration(800)
  ...
  }, [data, currentDayIndex, getSentimentColor, showTooltip, hideTooltip]);
  ...
  // Get unique dates from data - works for both mock and API data
  const uniqueDates =
    data && data.keywords
      ? [...new Set(data.keywords.map(kw => kw.date))].sort()
      : [];
  const currentDate =
    uniqueDates.length > 0 ? uniqueDates[currentDayIndex] : '';
      ...
      <div className="word-cloud-header">
        ...
        <div className="data-source-toggle">
          <label className="toggle-label">
            ...
            <span className="toggle-text">{'Real data'}</span>
          </label>
        </div>
      </div>
